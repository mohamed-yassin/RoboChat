...
function whatsapp_send_messege($sub,$parametars, $type = 'chat' )
{
    ...
    if($sub_connection_data['api'] == '' || $sub_connection_data['token'] == ''){
        return array('errors' => __('Please call admin to re-check your data','robo') ); 
    }
    ...
    if($sub_connection_data['msgs']  && $sub_connection_data['msgs'] >  0 ){
        ...
        if(isset($response['sent']) && $response['sent'] ==  1 ){ // sent successfully 
            ...
            $response['errors'] =  __('Please call admin to re-check your data','robo');
        }
    }else {
...
function whatsapp_log_out($api,$token,$sub = 0)
{
    ...
    if($sub != 0){
        echo '<a href="'.admin_url('admin.php?page='.$sub.'&process=log_out').'" class="button button-primary" >'.__('Close the Session','robo').'</a>';
    }else {
        return $result;
    }
}
...
function chat_api_main_processes(){
    ...
    if($process == 'connect' ){
        ...
        if(isset($result_array['accountStatus']) && $result_array['accountStatus'] == 'got qr code'){
            echo __('Scan the code, then press the button again','robo');
        ...
        echo __('Based on the chat API, operations will not be communicated on the server for 5 minutes from now and all API responses will be incomplete </br>','robo');
        ...
        echo  wp_update_post( $my_sub ) >  0  ? __('There is a problem with the update','robo') :  __('Subscription has been renewed','robo')  ;
    }

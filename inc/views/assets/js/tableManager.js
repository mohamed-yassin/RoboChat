    ...
    $.fn.tablemanager = function(options = null) {
        ...
        **/
        var Table = jQuery(this),
            Heads = jQuery(this).find('thead th'),
            tbody = jQuery(this).find('tbody'), 
            rows = jQuery(this).find('tbody tr'), 
            rlen = rows.length, 
            arr = [],
            cells, 
            clen;
        ...
        if (options !== null) {
            ...
            **/
            ...
            if (options.showrows != '' && typeof options.showrows !== undefined && options.showrows !== undefined) {
                ...
                for (i = 0; i < showrows.length; i++) {
                    jQuery('select#numrows').append(jQuery('<option>', {
                      value: showrows[i],
                      text: showrows[i]
                    }));
                ...
                var selectNumRowsVal = jQuery('select#numrows').val();
                numPerPage = selectNumRowsVal;
                // on select num rows change get value and call function
                jQuery('select#numrows').on('change', function() {
                    selectNumRowsVal = jQuery(this).val();
            ...
            **/
            if(options.appendFilterby === true || Table.hasClass('tableFilterBy')) {
                ...
                jQuery(this).before(filterbyDiv);

                // Populate select with every th text and as value use column number
                jQuery(Heads).each(function(i) {
                    if (!jQuery(this).hasClass('disableFilterBy')) {
                        jQuery('select#filter_by').append(jQuery('<option>', {
                          value: i,
                ...
                jQuery('input#filter_input').on('input', function() {
                    var val = $.trim(jQuery(this).val()).replace(/ +/g, ' ').toLowerCase();
                    var select_by = jQuery('select#filter_by').val();

                    Table.find('tbody tr').show().filter(function() {
                      // search into column selected by #filter_by
                      var text = jQuery(this).find('td:eq('+select_by+')').text().replace(/\s+/g, ' ').toLowerCase();
            ...
            **/
            if (options.dateFormat != '' && typeof options.dateFormat !== undefined && options.dateFormat !== undefined) {
                ...
                Heads.each( function(col) {
                    if (jQuery(this).data('tablemanager') && jQuery(this).data('tablemanager').dateFormat !== undefined) {
                        var dateF = jQuery(this).data('tablemanager').dateFormat;
                        dateColumn.push(col);
        ...
        **/
        Heads.each( function(n) {
            ...
            var disable = (jQuery(this).data('tablemanager') === 'disable') ? true : (jQuery(this).hasClass('disableSort') ? true : false);
            
            if(!disable === true) {
                jQuery(this).on('click', function() {
                    if (jQuery(this).hasClass('sortingAsc')) {
                        jQuery(Heads).removeClass('sortingAsc').removeClass('sortingDesc');
                        jQuery(this).addClass('sortingDesc');
                        order = 1;
                    } else {
                        jQuery(Heads).removeClass('sortingDesc').removeClass('sortingAsc');
                        jQuery(this).addClass('sortingAsc');
                        order = 0;
                    }
                ...
                jQuery(this).addClass('sorterHeader');
            };
        });
        ...
        function TableSort(rules) {
            ...
            for (i = 0; i < rules.length; i++) {
                ...
                if (i == 0) {
                    ...
                    jQuery(thead).addClass(firstClassOrder);
                }
                asc = (order == 0) ? 1 : -1;
                // if column is date
        ...
        **/
        function generatePaginationValues() {
            ...
            jQuery('.pagecontroller-num').eq(0).addClass('currentPage');
        ...
        **/
        function paginate(curPage = null, perPage = null) {
            ...
            Table.on("paginating", function(){
                jQuery(this).find('tbody tr').hide().slice(curPage * perPage, (curPage + 1) * perPage).show();
            });
        ...
        **/
        function appendPageControllers() {
            // reset div
            jQuery('#pagesControllers').html('');
            // First
            jQuery('#pagesControllers').append(jQuery('<button>', {
              value: 'first',
              text: '<<',
        ...
        **/
        function pagecontrollersClick() {
            jQuery('.pagecontroller').on('click', function() {
                // on click on button do something
                if (jQuery(this).val() == 'first') {
                    currentPage = 0;
                    paginate(currentPage,numPerPage);
                } else if (jQuery(this).val() == 'last') {
                    currentPage = numPages - 1;
                    paginate(currentPage,numPerPage);
                } else if ((jQuery(this).val() == 'prev')) {
                    if(currentPage != 0) {
                        currentPage = currentPage - 1;
                        paginate(currentPage,numPerPage);
                    }
                } else if (jQuery(this).val() == 'next') {
                    if(currentPage != numPages - 1) {
                        currentPage = currentPage + 1;
                        paginate(currentPage,numPerPage);
                    }
                } else {
                    currentPage = jQuery(this).val() - 1;
                    paginate(currentPage,numPerPage);
                }
                // Reset class and give to currentPage
                jQuery('.pagecontroller-num').removeClass('currentPage');
                jQuery('.pagecontroller-num').eq(currentPage).addClass('currentPage');
            });
        }
